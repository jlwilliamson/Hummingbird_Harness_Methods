...
date: "Created 4/3/2021; last revised 07/07/21 "
output: html_document
---

Script for Williamson & Witt, "A lightweight body harness for tracking hummingbirds", *Journal of Avian Biology*. 
...
Note that “Apparent” survival confounds surviving *and* remaining in the study area. In this script (for simplicity), I use “survival” when describing apparent survival.
...
# load in data 
```{R}
sl <- read.csv("0_raw_data/Table2_DigitalVersion_Hummingbird_Survival_Estimates_forR_04-15-21.csv", stringsAsFactors = TRUE) # Hummingbird survival rate lit review
cjs <- read.csv("0_raw_data/Patagona_Cormack-Jolly-Seber_06-18-21_ForDryad.csv", stringsAsFactors = TRUE) # capture history data 
...
Since Option 1 (subbing "111" in for one "011" level) is the most parsimonious solution, we'll proceed this way and assess "dummy levels" below (see diagnostic plots exploring these options).
...
same window of the breeding season. But, if we do want to play with time, capture interval is 1/12 months = ~0.08/month (so ~0.17 for 2 month interval and 0.83 for 10 months; see Blake and Loiselle 2008 cited in paper)
...
Rationale: ML searches seem to reach the same results whether or not this warning is thrown, as long as the ML parameter values actually are within the user-specified bounds.

Source info here: http://phylo.wikidot.com/biogeobears-warnings-to-ignore-mostly
...
- The top model is the simplest model, which assumes constant survival and detection probability. 
...
- These tests have VERY confusing names and confusing documentation (and online tutorials conflict w/ descriptions written about tests in Gimenez et al. 2018). Names and numbers do not go in order! i.e., Gimenez et al. describes running test2ct() and then test3sr() versus the two 'test2's and two 'test3's in order. 
...
# Figure 3: Plot of point estimate comparison of hummingbird survival rates from the literature
```{r}
...
Figure3 <- ggplot(data=sl, aes(x=common_name, y=phi)) + 
     ...
     # geom_point(mapping=aes(y=phi), size=5, shape=21, fill="black") + # Plot all points at once 
      ...
      theme(plot.margin = unit(c(0.2,0.2,0.2,0.4), "cm")) +  # top, right, bottom, left
          # ggtitle("(a)") + 
...
print(Figure3)
ggsave(Figure3, filename="Figure3_SurvivalRateComparison_07-07-21.pdf", height=7, width=9, units="in")
