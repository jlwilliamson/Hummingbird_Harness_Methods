...
But, before we move full steam ahead: 
...
# Make a boxplot with confidence intervals showing how estimates shift
```{R}
...
JESSIE NEEDS TO EDIT ALL BELOW TO MAKE SENSE /MAKE SURE CODE IS ANNOTATED FOR GIT UPLOAD
...
Let's try performing individual tests for the full dataset and data susets to better understand the components and how they’re related.
...
NOW **Test 3**, which tests whether marking affects survival (equal survival assumption). There are two components to Test 3: Test3.SR and Test3.SM.
...
sl$common.name <- factor(sl$common_name
...
# Phi (survival estimates) from all hummingbirds
# ggplot(sl, aes(x=common_name, y=phi)) + 
#       geom_errorbar(mapping=aes(ymin=phi_lci, ymax=phi_uci), width=0, size=4.1, color="grey60", alpha=0.5) + # CIs
#       geom_errorbar(mapping=aes(ymin=phi-phi_se, ymax=phi+phi_se), width=0.07, size=1, color="olivedrab3") + # se
#       geom_point(mapping=aes(y=phi), size=5, shape=21, fill="black") +
#      # coord_flip() + 
#       labs(x = "Species", y = "Survival Rate") +
#       theme(axis.text.x=element_text(angle=45, hjust=1.0)) + 
#     #  ggtitle("Survival Rate Comparison") +
#       theme_classic() 
# Width controls width of END CAPS on error bars 
# size controls size of points and lines

hum.survival <- ggplot(data=sl, aes(x=common.name, y=phi)) + 
      geom_errorbar(aes(ymin=phi_lci, ymax=phi_uci), width=0.0, size=4.1, color="grey84") +
      geom_errorbar(aes(ymin=phi-phi_se, ymax=phi+phi_se), width=0.13, size=1, color="gray28") + # purple 3 is pretty
      geom_point(data=sl[sl$sex=="female",], aes(y=phi, x=common.name), # Plot female points
            shape=16, color="goldenrod2", size=6) +
      geom_point(data=sl[sl$sex=="male",], aes(y=phi, x=common.name), # Plot male points another color 
            shape=16, color="turquoise3", size=6) + 
      geom_point(data=sl[sl$sex=="species-level",], aes(y=phi, x=common.name), # Plot species-level estimates in black
            shape=16, color="black", size=6) +
     # geom_point(mapping=aes(y=phi), size=5, shape=21, fill="black") + # Plot all points at once 
      scale_y_continuous(limits=c(0,1.0), breaks=c(0,0.25,0.5,0.75,1.0)) +
      scale_x_discrete(labels=c("Archilochus colubris [1]", "Archilochus colubris [1]",
                              "Archilochus colubris [2]", "Archilochus colubris [2]",
                              "Archilochus colubris [3]", "Archilochus colubris [3]",
                              "Archilochus colubris [4]", "Archilochus colubris [4]",
                              "Colibri thalassinus", "Hylocharis leucotis", "Phaethornis superciliosus",
                              "Phaethornis malaris [5]", "Phaethornis malaris [6]","Adelomyia melanogenys", 
                              "Heliodoxa jacula", "Phaethornis baroni","Augastes scutatus [7]", 
                              "Augastes scutatus [7]", "Patagona gigas")) +
      theme_classic() +     
      theme(axis.text.x=element_text(angle=45, hjust=1.0)) + 
          # ggtitle("(a)") + 
      theme(plot.title.position="plot", plot.title = element_text(face="bold")) + 
          # parameter "plot" specifies that you want "title" flush with y-axis; bold face makes panel header bold 
          # This is good for labeling figure panels! Avoids having to manually toy w/ hjust and vjust
      labs(x="Species", y = expression("Survival Estimate"~"("~italic(phi)~")")) + 
...
print(hum.survival)
ggsave(hum.survival, filename="hum.survival_LiteratureReviewPointEstimates.pdf", height=7, width=9, units="in")
# Charette et al. = [1]; Mulvihill et al. 1992 = [2]; Baumgartner 1989 = [3]; Hilton & Miller 2003 = [4]; Blake & Loiselle 2008 = [5]; Blake & Loiselle 2013 = [6]; Rodrigues et al. 2013 = [7]; Present study = [8]

 
# Is y-axis phi symbol italicized? 
```
